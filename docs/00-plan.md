...
| 6 | Tests on the matching + delivery logic | Primary validation tool for AI-generated code |
| 7 | Process artifacts: prompt history, decision log, AI review log, README | The actual submission |
| 8 | (Stretch) second event source (RSS) | Proves the `EventSource` abstraction is real, not decorative |
...
| M2 | Vertical slice: one simulated event → one alert rule → email adapter (dry-run) → delivery log row. Demoable. | 3h |
| M3 | Slack adapter via channel registry; matching rules widened (type, severity, keywords). | 1.5h |
| M4 | Admin view: list users/rules, delivery log, "inject event" button. | 2.5h |
| M5 | Tests on matcher + delivery orchestration; fix what they expose. | 1.5h |
| M6 | Process artifacts consolidated, README, final commit review. | 2h |
| M7 | Stretch: RSS event source. | if time |
